...
TEST(UtilTest, GPSScale) {
  ...
  double lat = ToRad(42.276617);
  double latscale, lonscale;
  GPSLatLonScale(lat, &latscale, &lonscale);
  EXPECT_NEAR(ToDeg(111079.0), latscale, 50.0);
  EXPECT_NEAR(ToDeg(82491.0), lonscale, 50.0);

  lat = ToRad(20.0);
  GPSLatLonScale(lat, &latscale, &lonscale);
  EXPECT_NEAR(ToDeg(110704.0), latscale, 100.0);
  EXPECT_NEAR(ToDeg(104646.0), lonscale, 100.0);
}
